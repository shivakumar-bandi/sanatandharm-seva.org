  ...
  const [image, setImage] = useState(null);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (festival && festival._id) {
                await axios.put(`${API_URL}/api/festivals/${festival.id}`, formData, {
    ...
    return (
        <div className="add-festival">
            <h2>{festival ? 'Edit Festival' : 'Add Festival'}</h2>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit">{festival ? 'Update Festival' : 'Add Festival'}</button>
            </form>
        </div>
    );
};
